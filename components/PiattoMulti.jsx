          ...
          <div className="flex justify-center space-x-10 items-center">
            ...
            {immaginiSezione[index] && (
              <img
                ...
                alt=""
                className="w-auto h-48 my-4"
              />
            )}

            {traduzioniSezione[index] && (
              <div className="text-26 font-semibold">{traduzioniSezione[index]}</div>
            )}
          </div>
            ...
            {elementiMenu[index]?.map((item, idx) => (
              <li key={idx} className="py-2">
                <div className="flex justify-between">
                  ...
                  <span className="font-semibold text-17">{item.prezzo_cibo}</span>
                  <span className="font-semibold text-17">{item.traduzione_cibo}</span>
                </div>
